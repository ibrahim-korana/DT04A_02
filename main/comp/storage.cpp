...
bool Storage::file_control(const char *name)
{
    if (!file_search(name)) {
        FILE *fd = fopen(name, "w");
        if (fd==NULL) {ESP_LOGE(STORETAG,"%s not created",name);return false;}
        fclose(fd);
        return false;
    }                      
  return true;
...
int Storage::file_size(const char *name)
{
    ...
    if (fd == NULL) {ESP_LOGE(STORETAG, "%s not open",name); return 0;}
    fseek(fd, 0, SEEK_END);
...
{
     FILE *fd = fopen(name, "r+");
     if (fd == NULL) {ESP_LOGE(STORETAG, "%s not open",name); return false;}
