...
void Dali::backword_yesno(backword_t data)
{
  ...
  //  gpio_set_level(_rx,1);
    ...
    while(ok)
    {
      if (gpio_get_level(_rx)==0) {ret=true;say++; ok=false;}
      ets_delay_us(10);
      if (gpio_get_level(_rx)==0) {ret=true;say++; ok=false;}
      if (++cnt>400) ok=false;
    }
    ...
    if (say>0) {printf(" say %d\n",say);say=0;}
    

   // portDISABLE_INTERRUPTS(); 
   // vTaskDelay(12/portTICK_PERIOD_MS); 
   // ets_delay_us(FULL_PERIOD*4);
...
#define KALIBRASYON 50

void Dali::end_clock(void *args)
{
    Dali *self = (Dali *)args;
    self->Olcum_Ready = false;
    static int bb = 0;  
